...
//  Motion
//
//  Full-screen Bayer dither with touch-to-reveal color for gameplay.
...
[[ stitchable ]] half4 ditherDashEffect(
    float2 position,
    half4 color,
    float pixelScale,
    float touchX,
    ...
    half4 ditherColor = half4(half(dithered), half(dithered), half(dithered), color.a);

    // Touch reveal
    if (touchActive > 0.5) {
        float2 touchPos = float2(touchX, touchY);
        float dist = distance(position, touchPos);
        float reveal = 1.0 - smoothstep(revealRadius * 0.4, revealRadius, dist);
